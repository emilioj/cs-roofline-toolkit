...
class ert_core:
  ...
  def flags(self):
    ...
    parser.add_option("--run",dest="run",action="store_true",default=True,help="Run the micro-kernels [default]")
    parser.add_option("--no-run",dest="run",action="store_false",default=True,help="Don\"t run the micro-kernels")

    parser.add_option("--post",dest="post",action="store_true",default=True,help="Run the post-processing [default]")
    parser.add_option("--no-post",dest="post",action="store_false",default=True,help="Don't run the post-processing")

    parser.add_option("--verbose",dest="verbose",action="store",nargs=1,default=1,type=int,help="Set the verbosity of the screen output [default = %default]")
  ...
  def configure(self):
    ...
    make_dir_if_needed(self.results_dir,"results",False)
        ...
        if mpi_procs * openmp_threads in procs_threads_list:
          ...
          if self.options.run:
            if os.path.exists("%s/run.done" % run_dir):
              ...
              if self.options.verbose > 0:
                print "    MPI %d, OpenMP %d" % (mpi_procs,openmp_threads)
